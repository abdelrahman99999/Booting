...
typedef struct {
    ...
    // function signatures
    size_t (*fread)(JANPATCH_STREAM*,void*, size_t,size_t* );
    size_t (*fwrite)(JANPATCH_STREAM *,const void*, size_t, size_t * );
    int    (*fseek)(JANPATCH_STREAM*, long int);
...
static size_t jp_fread(janpatch_ctx *ctx, void *ptr, size_t size, size_t count, janpatch_buffer *buffer) {
    ctx->fseek(buffer->stream, buffer->position);

    size_t bytes_read = ctx->fread(buffer->stream,ptr,count,&size );
    bytes_read=size;
...
static size_t jp_fwrite(janpatch_ctx *ctx, const void *ptr, size_t size, size_t count, janpatch_buffer *buffer) {
    ...
    size_t bytes_written = ctx->fwrite(buffer->stream,ptr,count,&size);
    bytes_written =size;
